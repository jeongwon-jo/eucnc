	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					//마지막
				?>
					<!--<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=1');" class="first"><img src="/image/common/first.svg" alt=""></a>-->
				<?
					//이전
					...
					?>
						<li class="prev-bt" onclick="get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$prev_list?>');">&lt;</li>
					<?
				}
			...
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					?>
						<li class="num" onclick="get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');"><?=$i?></li>
					<?
					}else{
					?>
						<li class="num active" onclick="get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');"><?=$i?></li>
					<?
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					?>
						<li class="next-bt" onclick="get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$next_list?>');">&gt;</li>
					<?
					// 처음
					...
					?>
						<!--<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$last_page?>');" class="last"><img src="/image/common/last.svg" alt=""></a>-->
					<?
				}
