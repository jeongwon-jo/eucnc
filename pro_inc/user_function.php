...
function kor_age(string $birthday) : int{
	$today = date('Ymd');
	$brithday = date('Ymd', strtotime($birthday));
	return abs(floor(($today - $brithday) / 10000));
}

function cell_format(string $cell) : string{
	$cell = preg_replace('/[^0-9]/', '', $cell);
	return preg_replace('/(^02.{0}|^01.{1}|^15.{2}|^16.{2}|^18.{2}|[0-9]{3})([0-9]+)([0-9]{4})/', '$1-$2-$3', $cell);
}

function time_format(int $record): string{
	$h = (int) ($record / 60 / 60);
	$m = ((int) ($record / 60)) - ($h * 60);
	$s = $record - (($h * 60 * 60) + ($m * 60));
...
function getDaysByDate(string $date): array{
	$select_date_timestamp = strtotime($date);
	$week_first_day_timestamp = strtotime(date('Y-m-d', $select_date_timestamp) . " -" . date("w", $select_date_timestamp) . "days +1 day");
	$week_first_day_format = date('Y-m-d', $week_first_day_timestamp);

	return array_map(static function(int $day) use ($week_first_day_format): string{
		return date("Y-m-d", strtotime("$week_first_day_format +{$day}days"));
	}, range(0, 6));
}
...
function uploadMultiFile($el_name, $_P_DIR_FILE, $index, $filename_org = ""){
	if($_FILES[$el_name]['name']){
		$file_name = $_FILES[$el_name]['name'][$index];
		$full_filename = explode(".", "$file_name");
		$extension = $full_filename[sizeof($full_filename) - 1];
		########## 등록한 파일이 업로드가 허용되지 않는 확장자를 갖는 파일인지를 검사한다. ##########
		if(!strcmp($extension, "html") || !strcmp($extension, "htm") || !strcmp($extension, "cgi") ||
		...
		$toName = $_P_DIR_FILE . $file_name;
		$fromName = $_FILES[$el_name]['tmp_name'][$index];
		if(!move_uploaded_file($fromName, $toName)){
			error_frame("UPLOAD_COPY_FAILURE");
			exit;
		}
...
function uploadFileThumb_1($HTTP_POST_FILES, $el_name, $el, $_P_DIR_FILE, $i_width = "", $i_height = "", $i_width2 = "", $i_height2 = "", $i_width3 = "", $i_height3 = "", $watermark_sect = ""){
	...
	if($_FILES[$el_name]['size'] > 0){
		...
		$thumbS_width = $i_width;
		$thumbS_height = $i_height; // 스몰섬네일 크기
		$thumbS_width2 = $i_width2;
		$thumbS_height2 = $i_height2; // 스몰섬네일 크기
		$thumbS_width3 = $i_width3;
		$thumbS_height3 = $i_height3; // 스몰섬네일 크기
...
function window_frame(string $url = "", string $errno = "") : void{
...
function mail_utf($from_email, $from_name, $to_email, $subject, $body, $file = ""){
	...
	if(!mail($to_email, $subject, $bodytext, $mailheaders, '-f' . $from_email)){
		return 0;
	}
	return 1;
}
...
function send_fcm_old($title, $message, $directURL, $id){
	...
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $keyword2s);
...
function hideEmail($email = ''){
	...
	/*
	$aEmail2 = explode(".", $aEmail['1']);
	$strEmail_2 = str_pad(substr($aEmail2['0'], 0, strlen($aEmail2['0'])/2), strlen($aEmail2['0']), '*');

	if (!empty($aEmail2['2'])) {
		return $strEmail_1 . '@' . $strEmail_2 . '.' . $aEmail2['1'] . "." . $aEmail2['2'];
	} else {
		return $strEmail_1 . '@' . $strEmail_2 . '.' . $aEmail2['1'];
	}
	*/
