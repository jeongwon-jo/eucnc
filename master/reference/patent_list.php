...
if($keyword){
	if($keyword_type){
		switch($keyword_type){
			case "subject":
				$where .= " AND ( 
					`kor_subject_1` LIKE '%$keyword%' OR
					`kor_subject_2` LIKE '%$keyword%' OR
					`eng_subject_1` LIKE '%$keyword%' OR
					`eng_subject_2` LIKE '%$keyword%' 
				) ";
				break;
			case "number":
		...
		$where .= "AND ( 
						...
						`kor_subject_2` LIKE '%$keyword%' OR
						`eng_subject_1` LIKE '%$keyword%' OR
						`eng_subject_2` LIKE '%$keyword%' OR `number` LIKE '%$keyword%' ) ";
	}
}

$query = " SELECT * FROM patent_info WHERE $where ORDER BY `priority` ASC LIMIT $StarRowNum,$EndRowNum ";
$result = mysqli_query($gconnet, $query);

$query_cnt = " SELECT idx FROM patent_info WHERE $where ";
...
<script>
    ...
    function go_write(){
        location.href = "./patent_write.php?<?=$total_param?>"
    }

    function go_modify(idx) {
        location.href = "./patent_modify.php?<?=$total_param?>&idx=" + idx
    }

    function go_remove(){
        const links = getSelectedCheckboxIds();
        if(Object.keys(links).length === 0){
            alert("삭제할 특허 데이터를 선택해주세요.")
            return
        }
...
<body>
	<form action="./patent_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<form name="priority_frm" action="./global_table_priority_update_action.php" target="_fra_admin">
		...
		<input type="hidden" id="tb_name" name="tb_name" value="patent_info">
	</form>
				...
				<div class="content">
					...
					<div class="list">
								...
								<tr>
									...
									<td colspan="2">
										<select name="keyword_type">
											...
											<option value="number" <? if($keyword_type === 'number'){ echo 'selected'; } ?>>번호</option>
										</select>
									</td>
									<th scope="row">키워드</th>
									<td colspan="2">
						...
						<div class="search_wrap">
							<div class="result">
								<p class="txt">등록한 특허 목록 (<span><?=$num?></span>건)</p>
								<div class="btn_wrap">
							...
							<table class="search_list">
								...
								<tr>
									...
									<th scope="col">설명(국문)</th>
									<th scope="col">설명(영문)</th>
									<th scope="col">번호</th>
								...
								for($i = 0, $iMax = mysqli_num_rows($result); $i < $iMax; $i ++){
									...
									<tr>
										...
										<td><?=$listnum?></td>
										<td><?=$row['kor_subject_1']?><br><?=$row['kor_subject_2']?></td>
										<td><?=$row['eng_subject_1']?><br><?=$row['eng_subject_2']?></td>
										<td><?=$row['kor_desc_1']?><br><?=$row['kor_desc_2']?></td>
										<td><?=$row['eng_desc_1']?><br><?=$row['eng_desc_2']?></td>
										<td><?=$row['number']?></td>
										<td><?=$row['file_org']?></td>
										<td>
											<?
