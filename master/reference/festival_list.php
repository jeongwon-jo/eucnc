...
$query = " SELECT * FROM festival_info WHERE $where ORDER BY `priority` ASC LIMIT $StarRowNum,$EndRowNum ";
$result = mysqli_query($gconnet, $query);

$query_cnt = " SELECT idx FROM festival_info WHERE $where ";
...
<script>
    ...
    function go_write(){
        location.href = "./festival_write.php?<?=$total_param?>"
    }

    function go_modify(idx) {
        location.href = "./festival_modify.php?<?=$total_param?>&idx=" + idx
    }

    function go_remove(){
        const links = getSelectedCheckboxIds();
        if(Object.keys(links).length === 0){
            alert("삭제할 기업행사 데이터를 선택해주세요.")
            return
        }
...
<body>
	<form action="./festival_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<form name="priority_frm" action="./global_table_priority_update_action.php" target="_fra_admin">
		...
		<input type="hidden" id="tb_name" name="tb_name" value="festival_info">
	</form>
							...
							<table class="search_list">
								...
								for($i = 0, $iMax = mysqli_num_rows($result); $i < $iMax; $i ++){
									...
									<tr>
										...
										<td><?=$listnum?></td>
										<td><?=$row['kor_subject']?></td>
										<td><?=$row['eng_subject']?></td>
										<td><?=$row['file_org']?></td>
										<td>
											<?
											if($iTotalSubCnt > 1){
