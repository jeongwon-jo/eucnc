...
$query = " SELECT * FROM certification_info WHERE $where ORDER BY `priority` ASC LIMIT $StarRowNum,$EndRowNum ";
$result = mysqli_query($gconnet, $query);

$query_cnt = " SELECT idx FROM certification_info WHERE $where ";
...
<script>
    ...
    function go_write(){
        location.href = "./certification_write.php?<?=$total_param?>"
    }

    function go_modify(idx) {
        location.href = "./certification_modify.php?<?=$total_param?>&idx=" + idx
    }

    function go_remove(){
        const links = getSelectedCheckboxIds();
        if(Object.keys(links).length === 0){
            alert("삭제할 인증서 데이터를 선택해주세요.")
            return
        }
...
<body>
	<form action="./certification_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<form name="priority_frm" action="./global_table_priority_update_action.php" target="_fra_admin">
		...
		<input type="hidden" id="tb_name" name="tb_name" value="certification_info">
	</form>
				...
				<div class="content">
					...
					<div class="list">
						...
						<div class="search_wrap">
							<div class="result">
								<p class="txt">등록한 인증서 목록 (<span><?=$num?></span>건)</p>
								<div class="btn_wrap">
