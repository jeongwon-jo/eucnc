...
$where = " 1 = 1 ";
if($keyword){
	$where .= " AND ( `kor` LIKE '%$keyword%' OR `eng` LIKE '%$keyword%' ) ";
}

$query = " SELECT * FROM awards_info WHERE $where ORDER BY `priority` ASC LIMIT $StarRowNum,$EndRowNum ";
...
<script>
    ...
    function go_write(){
        location.href = "./awards_write.php?<?=$total_param?>"
    }

    function go_modify(idx) {
        location.href = "./awards_modify.php?<?=$total_param?>&idx=" + idx
    }

    function go_remove(){
        const links = getSelectedCheckboxIds();
        if(Object.keys(links).length === 0){
            alert("삭제할 수상내역 데이터를 선택해주세요.")
            return
        }
...
<body>
	<form action="./awards_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<form name="priority_frm" action="./global_table_priority_update_action.php" target="_fra_admin">
		...
		<input type="hidden" id="tb_name" name="tb_name" value="awards_info">
	</form>
				...
				<div class="content">
					...
					<div class="list">
						...
						<div class="search_wrap">
							<div class="result">
								<p class="txt">등록한 수상내역 목록 (<span><?=$num?></span>건)</p>
								<div class="btn_wrap">
