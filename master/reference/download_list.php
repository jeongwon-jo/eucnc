...
<script>
    ...
    function go_write(){
        location.href = "./download_write.php?<?=$total_param?>"
    }

    function go_modify(idx) {
        location.href = "./download_modify.php?<?=$total_param?>&idx=" + idx
    }

    function go_remove(){
        const links = getSelectedCheckboxIds();
        if(Object.keys(links).length === 0){
            alert("삭제할 다운로드 데이터를 선택해주세요.")
            return
        }
...
<body>
	<form action="./download_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<form name="priority_frm" action="./global_table_priority_update_action.php" target="_fra_admin">
		...
		<input type="hidden" id="tb_name" name="tb_name" value="download_info">
	</form>
				...
				<div class="content">
					...
					<div class="list">
						...
						<div class="search_wrap">
							<div class="result">
								<p class="txt">등록한 다운로드 게시글 목록 (<span><?=$num?></span>건)</p>
								<div class="btn_wrap">
								...
								for($i = 0, $iMax = mysqli_num_rows($result); $i < $iMax; $i ++){
									...
									<tr>
										...
										<td>
											...
											if($iTotalSubCnt > 1){
												if($i === 0){ ?>
													<a href="javascript:;" onclick="go_update('<?=$row['idx']?>', 'down')">▼</a>
												<? }elseif($i === $iTotalSubCnt-1){ ?>
													<a href="javascript:;" onclick="go_update('<?=$row['idx']?>', 'up')">▲</a>
												<? }else{ ?>
													<a href="javascript:;" onclick="go_update('<?=$row['idx']?>', 'down')">▼</a>
													<a href="javascript:;" onclick="go_update('<?=$row['idx']?>', 'up')">▲</a>
												<? } ?>
