...
if($keyword){
	if($keyword_type){
		switch($keyword_type){
			...
			case "writer": $where .= " AND `writer` LIKE '%$keyword%' "; break;
		}
	}else{
		$where .= " AND (`subject` LIKE '%$keyword%' OR `content` LIKE '%$keyword%' OR `writer` LIKE '%$keyword%') ";
	}
}
...
<script>
    ...
    function go_write(idx){
        location.href = "./inquiry_write.php?<?=$total_param?>&idx=" + idx
    }

    function go_modify(idx) {
        location.href = "./inquiry_modify.php?<?=$total_param?>&idx=" + idx
    }
...
<body>
	<form action="./inquiry_remove_action.php" target="_fra_admin" name="remove_frm" method="post">
	...
	<div id="wrap" class="skin_type01">
		...
		<div class="sub_wrap">
			<? include $_SERVER["DOCUMENT_ROOT"]."/master/include/inquiry_left.php"; // 좌측메뉴?>
			<!-- content 시작 -->
				...
				<div class="content">
					...
					<div class="list">
						...
						<div class="search_wrap">
							<div class="result">
								<p class="txt">온라인 문의 목록 (<span><?=$num?></span>건)</p>
								<div class="btn_wrap">
							...
							<table class="search_list">
								...
								for($i = 0, $iMax = mysqli_num_rows($result); $i < $iMax; $i ++){
									...
									<tr>
										...
										<td><?=$row['subject']?></td>
										<? if($row['state'] == 0){ ?>
											<td><a href="javascript:go_write('<?=$row['idx']?>');" class="btn_gray">답변 등록</a></td>
										<? }else{ ?>
											<td><a href="javascript:go_modify('<?=$row['idx']?>');" class="btn_gray">답변 수정</a></td>
										<? } ?>
									</tr>
								<? } ?>
								</tbody>
